              ...
              <form action="" method="POST">
                <div class="row">
                  <div class="form-group col-md-2">
                    ...
                    <option value="">PIlih Kelas</option>
                      <?php 
                      $kls = postOrOr('kelas');
                      $q = mysqli_query($koneksi, "SELECT tk.* FROM tb_kelas tk JOIN tb_jadwal tj ON tk.id=tj.id_kelas WHERE tj.nip='$_SESSION[nip]'");
                      foreach ($q as $kl) {
                      ?>
                      <option value="<?= $kl['id'] ?>" <?= ($kls == $kl['id'] ? 'selected' : '') ?> jurusan="<?= $kl['tipe_kelas'] ?>"><?= $kl['nama'] ?></option>  
                      <?php } ?>
                    </select>
                  </div>
                  <?php 
                ...
                <div class="form-group col-md-2">
                    ...
                    <option value="">PIlih Mata Pelajaran</option>
                      <?php 
                      $map = postOrOr('mapel');
                      $qm = mysqli_query($koneksi, "SELECT tm.* FROM tb_mapel tm JOIN tb_jadwal tj ON tm.id=tj.id_mapel WHERE tj.nip='$_SESSION[nip]'");
                      foreach ($qm as $m) {
                      ?>
                      <option value="<?= $m['id'] ?>" <?= ($map == $m['id'] ? 'selected' : '') ?>><?= $m['nama'] ?></option>  
                      <?php } ?>
                    </select>
                  </div>
                  
                  <div class="form-group col-md-4">
                        <input type="date" name="start_date" value="<?= date("Y-m-d") ?>" placeholder="tanggal awal" class="form-control" value="<?= postOrOr('start_date') ?>">
                  </div>
                <?php
                  }
                  ?>
            ...
            <table class="table table-bordered dataTable" id="dataTable">
                ...
                <?php
                   if(isset($d['kelas'])){
                    ...
                    // echo "SELECT ts.nama, ts.nis, ta.tanggal,ta.status_absen, ta.id_kelas, tm.nama as mapel FROM tb_absen_pelajaran ta JOIN tb_siswa ts on ta.nis=ts.nis JOIN tb_mapel tm ON ta.id_mapel=tm.id $where GROUP BY ta.id_mapel";
