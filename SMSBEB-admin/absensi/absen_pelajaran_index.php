...
$kelass = mysqli_fetch_array(mysqli_query($host, "SELECT * FROM tb_kelas WHERE id_kelas = '$idKelas'"));
$hs = mysqli_query($host, "SELECT * FROM tb_kelas INNER JOIN tb_mapel ON tb_mapel.jurusan = tb_kelas.tipe_kelas WHERE tb_kelas.id_kelas = '$idKelas'");
      ...
      <div id="content">
        ...
        <div class="container-fluid">
          ...
          <div class="card shadow mb-4">
            ...
            <div class="card-body">
                <?php while($mapelK = mysqli_fetch_array($hs)){ ?>
                <div class="card-body">
                    <div class="row">
                        <div class="col-md-10">
                            Absen Mata Pelajaran : <?= $mapelK["nama_mapel"] ?>
                        </div>
                        <div class="col-md-2">
                            <?php 
                               ...
                               if(isset($_GET["tanggal"])){
                                ...
                                $rQ = mysqli_query($host, "SELECT * FROM tb_absen_pelajaran WHERE id_kelas = '$t' AND id_mapel = '$mapelApel' AND DATE(timestamp) = '$tanggal'");
                                $date = $tanggal;
                               }else{
                                   $rQ = mysqli_query($host, "SELECT * FROM tb_absen_pelajaran WHERE id_kelas = '$t' AND id_mapel = '$mapelApel' AND DATE(timestamp) = '$date'");
                               }

                                
                                if(mysqli_num_rows($rQ) != 0){
                                    ?>
                                <a href="absen_pelajaran_edit.php?id_kelas=<?= $mapelK["id_kelas"]?>&tanggal=<?= $date ?>&mapel=<?= $mapelApel ?>" class="btn btn-info">Edit</a>
                                <?php } else { 
                                    if($interval == 0){
                                        ?>
                                      <a href="absen_pelajaran_tambah.php?id_kelas=<?= $mapelK["id_kelas"]?>&mapel=<?= $mapelApel ?>" class="btn btn-success">Tambah</a>  
                                    <?php }
                                    }
                                    ?>
